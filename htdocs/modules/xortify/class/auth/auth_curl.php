...
include_once ICMS_ROOT_PATH . '/modules/xortify/class/auth/auth_curl_provisionning.php';
...
class XortifyAuthCurl extends XortifyAuth {
	...
	function authenticate($uname, $pwd = null) {
		$authenticated = false;
		$this->XortifyAuthCurl($GLOBALS['db']);
		...
		curl_setopt($this->curl_client, CURLOPT_POSTFIELDS, array('xoops_authentication' => json_encode(array("username"=> $this->curl_icms_username, "password"=> $this->curl_icms_password, "auth" => array('username' => $uname, "password" => $pwd, "time" => time(), "passhash" => sha1((time()-$rnd).$uname.$pwd), "rand"=>$rnd)))));
	...
	function validate($uname, $email, $pass, $vpass){
		$this->XortifyAuthCurl($GLOBALS['db']);
		$rnd = rand(-100000, 100000000);
		curl_setopt($this->curl_client, CURLOPT_POSTFIELDS, array('xoops_user_validate' => json_encode(array("username"=> $this->curl_icms_username, "password"=> $this->curl_icms_password, "validate" => array('uname' => $uname, "pass" => $pass, "vpass" => $vpass, "email" => $email, "time" => time(), "passhash" => sha1((time()-$rnd).$uname.$pass), "rand"=>$rnd)))));
		$data = curl_exec($this->curl_client);
		curl_close($this->curl_client);
		$result = $this->obj2array(json_decode($data));	
		if ($result['ERRNUM']==1){
	...
	function network_disclaimer(){
		$this->XortifyAuthCurl($GLOBALS['db']);
		curl_setopt($this->curl_client, CURLOPT_POSTFIELDS, array('xoops_network_disclaimer' => json_encode(array("username"=> $this->curl_icms_username, "password"=> $this->curl_icms_password))));
		$data = curl_exec($this->curl_client);
		curl_close($this->curl_client);
