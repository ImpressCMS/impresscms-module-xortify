...
include_once ICMS_ROOT_PATH . '/modules/xortify/class/auth/auth_json_provisionning.php';
...
class XortifyAuthJson extends XortifyAuth {
	...
	function authenticate($uname, $pwd = null) {
		...
		$data = file_get_contents(XORTIFY_JSON_API.'?icms_authentication='.urlencode(json_encode(array("username"=> $this->json_icms_username, "password"=> $this->json_icms_password, "auth" => array('username' => $uname, "password" => $pwd, "time" => time(), "passhash" => sha1((time()-$rnd).$uname.$pwd), "rand"=>$rnd)))));
	...
	function validate($uname, $email, $pass, $vpass){
		$rnd = rand(-100000, 100000000);	
		$data = file_get_contents(XORTIFY_JSON_API.'?icms_user_validate='.urlencode(json_encode(array("username"=> $this->json_icms_username, "password"=> $this->json_icms_password, "validate" => array('uname' => $uname, "pass" => $pass, "vpass" => $vpass, "email" => $email, "time" => time(), "passhash" => sha1((time()-$rnd).$uname.$pass), "rand"=>$rnd)))));
			$result = $this->obj2array(json_decode($data));	
		if ($result['ERRNUM']==1){
			return $result["RESULT"];
		} else {
	...
	function network_disclaimer(){
		...
		$data = file_get_contents(XORTIFY_JSON_API.'?icms_network_disclaimer='.urlencode(json_encode(array("username"=> $this->json_icms_username, "password"=> $this->json_icms_password))));
			$result = $this->obj2array(json_decode($data));	
	...
	function create_user($user_viewemail, $uname, $email, $url, $actkey, 
		...
		$rnd = rand(-100000, 100000000);
		$data = file_get_contents(XORTIFY_JSON_API.'?icms_create_user='.urlencode(json_encode(array("username"=> $this->json_icms_username, "password"=> $this->json_icms_password, "user" => array('user_viewemail' =>$user_viewemail, 'uname' => $uname, 'email' => $email, 'url' => $url, 'actkey' => $actkey, 'pass' => $pass, 'timezone_offset' => $timezone_offset, 'user_mailok' => $user_mailok, "time" => time(), "passhash" => sha1((time()-$rnd).$uname.$pass), "rand"=>$rnd), "siteinfo" => $siteinfo))));
			$result = $this->obj2array(json_decode($data));	
